<template>
    <div v-if="isExileModalVisible" class="modal" @click="closeModal">
      <div class="modal-content" @click.stop>
        <h2 v-if="sortedCards">{{sortedCards.length}} card<span v-if="sortedCards.length > 1">s</span></h2>
        <div class="card-grid">
          <div v-for="(card, index) in sortedCards" :key="index" class="exile-view-card" @mouseover="hover = true" @mouseleave="hover = false">
  ...
  <script>
  export default {
    emits: ["close-exile-modal", "add-card-to-hand"],
    props: {
      isExileModalVisible: Boolean,
      cards: Array
    },
    data() {
      return {
  ...
  .exile-view-card:hover .add-to-hand-button {
    opacity: 1;
  }
  </style>

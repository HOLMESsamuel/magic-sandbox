<template>
    <div v-if="isScrapModalVisible" class="modal" @click="closeModal">
      <div class="modal-content" @click.stop>
        <h2 v-if="sortedCards">{{sortedCards.length}} card<span v-if="sortedCards.length > 1">s</span></h2>
        <div class="card-grid">
          <div v-for="(card, index) in sortedCards" :key="index" class="scrap-view-card" @mouseover="hover = true" @mouseleave="hover = false">
  ...
  export default {
    emits: ["close-scrap-modal", "add-card-to-hand"],
    props: {
      isScrapModalVisible: Boolean,
      cards: Array
    },
    data() {
      return {
  ...
  .scrap-view-card:hover .add-to-hand-button {
    opacity: 1;
  }
  </style>
